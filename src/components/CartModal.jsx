...
import { cartUpdate, updatedCartItem } from "../store/customCartActions";
...
export const CartModal = ({ open, onClose }) => {
    ...
    return createPortal(
        <dialog className="w-[90%] max-w-xl m-auto rounded-lg p-4 bg-amber-200 text-slate-900 [&::-webkit-backdrop]:bg-black/60 [&::backdrop]:bg-black/60 shadow-2xl border-none" ref={dialogRef} onClose={onClose}>
            ...
            <div className="flex flex-col space-y-3 py-4 list-none md:text-lg">
            ...
            <p className="text-right font-bold text-lg">Total Amount: ${totalAmt}</p>
            <div className="flex justify-end gap-4 mt-6">


                {cartItems.length > 0 ? <>
                    <button type="button" className="text-lg font-medium py-1 px-4 cursor-pointer" onClick={onClose}>Close</button>
                    <button type="button" className="text-lg font-medium py-1 px-4 bg-amber-950 text-white rounded-md cursor-pointer hover:bg-amber-900 transition-colors duration-100">Checkout</button>
                </> :
